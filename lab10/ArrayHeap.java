...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        ...
        int minIndex = min(min(leftIndex, rightIndex), index);
        if (index != minIndex) {
            swap(index, minIndex);
            sink(minIndex);
        }
    ...
    public void changePriority(T item, double priority) {
        for (int i = 1; i <= size; i++) {
            if (contents[i].item().equals(item)) {
                ...
                int leftSon = leftIndex(i);
                int rightSon = rightIndex(i);
                if (i == min(parent, i)) {
                    swim(i);
                    break;
                }
                if (leftSon == min(leftSon, i) || rightSon == min(rightSon, i)) {
                    sink(i);
                    break;
                }
            }
